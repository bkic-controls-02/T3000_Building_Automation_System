...
/*
//S2010 编Program Files\Microsoft Visual Studio 10.0\VC\bin\cvtres.exe 来确保用
...
1. Setting 
2. ISP tool ifi 设
3. T3000 TSTAT10 WIFI 连及过R5 使用 Mus 和MSTP ， 好议
4. odbus 485能cnet 的大多
5. Schedule 无py Monday to Friday 的功能 为
6. TSTATAQ  multiple sensor 修O2 sensor 以及sensor 的 开间
7. 3000 底口 能更顺畅的 aebus 的设备通
...
1.odbus 底务次
2.O2 Node 温
3.O2 Net "Min Out Scale" "Max Out Scale" 无to manual TBD)

2019 03 13
1.oad prg schedule 不
2.ustom Unit 正正

2019 03 08
1.WMTRANDUCER output 个
2.Asix 的  整ogram   大于00 时， 户无
...
2019 03 01
1. program 中save  load  某些ogram decode error的问题
2. ProgramEditDebug 示改，HOL.
3. pdate 引动中替换原有xe 文件.


2018 12 21
1. ScreenEdit Sch 以l 可以OFF 值
2. AR1 AR2 CH的 cbo 中
3. BTU 界创建sourcefile 的文件文
4. Zigbee Pan ID   for TSTAT8 

2018 12 11
1. O2 NET 无DBUS_DEW_PT  的值
2. O2  Output2  value为
3. Graphic view ,改2

2018 11 27
1. Setting Done 不
2. Setting Show LCD B 有
3. Monitor D卡状
...
4.Bacnet Tool 多个
5.el时 哪pm是选中状态 
6.etwork point 中ubpanel 为0 的问
7.e ，不再会进
8.creen 中utput 和viable  由 to 改为mal时  点法更 值的问题.
...
1.
2.000 默功
3.在stom  range是 ditalrange 意外弹出
4.3BB同

2018-04-02  Update by Fance
1. T3-6CTA 通DBUS 485 正常
2.  多改底动动
...
1.部nput output debug 模式
2.TSTAT8  setpoint ，
3. T3000t 端 端口了
4. T3controller 修CD 显示

2018-0130 Update by Fance
1.g文a 的方式.
2.ubpanel 为，以 一样.
3.toorbarenu 新twork 和pel 的图
4.minipanel P地址化不
5. 备，
...
1. inipanel界tat的scdule界面
2. 复
3.T3000acnetois
4.在I的iut界面T8的inp界面切换
5.inipanel ARM 版m0口支持
...
1. Fix 0的时候
2. nd log 里面gital 个数 位

2017 - 01 - 16 Update by Fance
1. Var custmer units 客
2. PRG 入
3. creen edit 中作
...
1. ogram 时IME-ON 或者E-OFF  个数超过 T( ) 里面的的值显示异
...
2.  nput 的
3. Download file 自动备
4. STAT Scan , 需nel 下面的所有口
5. Update.exe 认

2016 - 11 - 04 Update by Fance
1. Settingdate 3秒后ing 界面
2. 冲突

2016 - 10 - 10 Update by Fance
1.的，panel界面在o 切换到T3在理input.
2016 - 09 - 09 Update by Fance
1. Bacnet view Setting UI , 有下
2. Downloadfile 传小.5大大加快传
...
1. inipanel 时e 无法正常显示的问题
2.   F 55 命令，让Mini立刻去扫描下面的 设
3. nge的时面
...
3. BB 在线播;
4. Building 
5. ScreenEdit 客时候用 t icon.
6. Analog custom tabel nput 
7. c的 崩溃问题;

2016 - 06 - 03 Update by Fance
1. rend log graphic 宽在绝

2016 - 06 - 01 Update by Fance
1. program edit to definition ,可以查a
2. edit debug 界刷iar;
3. 的
4. 数丢失
5. Graphic Zoom in /Zoom out 不
6. Graphic  rg1 后在锁
2016 - 05 - 31 Update by Fance
1. Graphic 前

2016 - 05 - 30  Update by Fance
1. Output PWM 在ital的时候 这一列;
2. 
...
1.成功后
2.Settingp修
3.Building 区ote device; remote 和本地示
4.Graphic igital 部间
5.Graphic 刻度 时1取

2016 - 05 - 23 Update by Fance
1. Cus Analog range 单
...
1.T3 -22i 1-11nput high speed count 功

2016 - 05 - 10 Update by Fance
1.Bacnet Setting  加me update 最后
2.input output 点nu时 加数

2016 - 05 - 04 Update by Fance
1.bject instance;改为节
2.后
3.

2016 - 04 - 29 Update by Fance
1. Output table 宽示;
2. Range ON/OFF 和FF/ON 用两io button 来表示;复 定在取
...
1. nipanel 界
2. raphic 界,4个np label 位置调整至左

2016 - 04 - 19 Update by Fance
1. ogram debug 界nrt键后 口的题
2. abel 的o面 解析 认为是的题a
3. ebug acnet 的 调
...
1. T3dbus 寄存mnel 界面一样,nl 寄存器 ，建立一据应
2. 艹困死

2016 - 1 - 8 Update by Fance
1. Setting ealth ，新valu
2. ，or 一样 ，用p图片;

2016 - 1 - 5 Update by Fance
1. Input external show T3 模块的

2016 - 1 - 5 Update by Fance
1.  Big  input27 - 32  small 11 - 16  tiny 6 - 11 range 他的 n低速脉冲;
...
1. Program IDE 
2. Program IDE color 的efault value.
...
1. program IDE debug window , 因换G
2. raphic window 点graphic 时 ，会自大
...
3. T3000 will scan device via serial port with baud rate both 600”nd “100”.Bore we only use 19200.
...
static int old_selected_item = WINDOW_PROGRAM; // 一用于 快捷键;
...
int click_resend_time = 0;//时 发送wi;
...
HANDLE connect_mstp_thread = NULL; // TP的程
HANDLE read_rs485_thread = NULL; // RS485用
HANDLE write_indb_thread = NULL; //
int connect_way = 0;  // 1 ODBUS RS485    2 MODBUS TCP
HANDLE hbip_whois_thread = NULL; // am 线程
...
LRESULT CDialogCM5_BacNet::Change_Next_Panel(WPARAM wParam,LPARAM lParam)
{
	...
	if(temp_direction == LIST_UP)
	{
		for (int i=0;i< pFrame->m_product.size();i++)
		{
			...
			if(pFrame->m_product.at(i).status == false)//nel 不续
			{
				continue;
			}
...
//	rc.top += 20;
	...
	//::GetWindowRect(BacNet_hwd,&temp_mynew_rect);	//view的
	...
	//Input_Window->m_input_list.SetFocus(); 免
...
void CDialogCM5_BacNet::Fresh()
{
	...
	g_bPauseMultiRead = true; // nipanel的停器个
...
//	g_NEED_MULTI_READ = FALSE;
	...
	else if(selected_product_Node.protocol == MODBUS_BACNET_MSTP)
	{
        ...
        Variable_Window->SetTimer(1, 10000, NULL);  //TP协
		...
		if(g_bac_instance>0)
			Send_WhoIs_Global(g_bac_instance, g_bac_instance); //2023 03 13 H  干扰 一大的
			//Send_WhoIs_Global(-1, -1);


		::PostMessage(BacNet_hwd,WM_DELETE_NEW_MESSAGE_DLG,START_BACNET_TIMER,0);
		//SetTimer(BAC_TIMER_2_WHOIS,20000,NULL);//于;不知道设备什
		return;
	}
	//else if((selected_product_Node.protocol == MODBUS_RS485) && (selected_product_Node.NetworkCard_Address.IsEmpty()))  // 2020 07 23 在
	...
	//
	//SetTimer(1,500,NULL);
	//SetTimer(BAC_TIMER_2_WHOIS,180000,NULL);//于;不知道设备什
...
#endif
	int ret = 0;
	if (ValidAddress(nconnectionip)==FALSE)  // C的
	{
	...
	if(!offline_mode)
	{
        ...
        ret = 1;
        if (MODE_SUPPORT_PTRANSFER != 1)  //Ptransfer 08端口访带Wi;
        {
            for (int i = 0;i<3;i++)
            {
	...
	if(ret)
	{
        ...
        //if (selected_product_Node.protocol != PROTOCOL_BIP_TO_MSTP)  // oinstance
        {
			...
			if (multy_ret == -12)//is，可能任务挂掉了
			{
				...
				if (temp_check_again == -12)
				{
					...
					if (is_local_temco_net) //Temco dg窗口 用师
					{
						CShowMessageDlg dlg;
            ...
            if ((selected_product_Node.nhardware_info & 0x02) == 2) //it1 是i模块;
            {
                ;
                //部分的 延适
            }
...
#endif 
		{
			...
			else//是扫描到得ba，因为不知道anel number有读到后更新数据库;
			{
	...
	else
	{
        ...
        for (int x = 0; x < 5; x++)
        {
            selected_product_Node.status_last_time[x] = false;//态都设
        }
	...
	if((g_bac_read_type == TYPE_ALL) || (g_bac_read_type == TYPE_SETTING) || g_bac_need_read_setting)
	{
		for (int i=0;i<1;i++)	
		{
			...
			else
			{
				...
				g_bac_need_read_setting = false;	//用读Inpu
			}
		}
...
void CDialogCM5_BacNet::OnTimer(UINT_PTR nIDEvent)
{
	...
	switch(nIDEvent)
	{
		if (hwait_write_thread != NULL) //     og的数据  
			break;
...
void	CDialogCM5_BacNet::Initial_Some_UI(int ntype)
{
	...
	Output_Window->Reload_Unit_Type();
	Inital_Tab_Loaded_Parameter();
	//PostMessage(WM_FRESH_CM_LIST,MENU_CLICK,TYPE_READ_CUSTOMER_UNIT);//tomer unit ，nge里面
	...
	{	
		...
		if ((g_selected_product_id == PM_ESP32_T3_SERIES) &&
			((int)Device_Basic_Setting.reg.pro_info.firmware0_rev_main) * 10 + (int)Device_Basic_Setting.reg.pro_info.firmware0_rev_sub >= ESP32_IO_COUNT_REDEFINE_VERSION)
		{
			//nput output  variable  esp32 的3动
			ReInital_Someof_Point();
			last_inital_point = 2; //  的cot
		}
		else
		{
	...
	//if((!read_customer_unit) && selected_product_Node.protocol != MODBUS_RS485    20200820 在直 以前是 第一遍
...
DWORD WINAPI RS485_Read_Each_List_Thread(LPVOID lpvoid)
{
    ...
    {
        output_reg = 15; //nipanel 的所
        input_reg = 15;
        variable_reg = 26;
        pid_con_reg = 3;
        schedule_list_reg = 2;   //15808 - 15955+21
        schedule_time_reg = 1;  //72包
    }
    ...
    {
        //output 45 6算*64  + input 46  *64  需要;
        ...
        if (read_result)
        {
            ...
            for (int i = 0;i < BAC_OUTPUT_ITEM_COUNT;i++)
            {
                ...
                memcpy(&m_Output_data.at(i), &read_data_buffer[i * 23], sizeof(Str_out_point));//utput 只个字到1个 b的寄存器里面;
            }
    ...
    case READINPUT_T3000/*BAC_IN*/:   //IN
    {
        ...
        if (read_result)
        {
            ...
            for (int i = 0;i < BAC_INPUT_ITEM_COUNT;i++)
            {
                ...
                memcpy(&m_Input_data.at(i), &read_data_buffer[i * 23], sizeof(Str_in_point));//nput 只个字到1个 b的寄存器里面;
            }
            CString str_serialid;
            str_serialid.Format(_T("%u"), selected_product_Node.serial_number);
            if (Bacnet_Private_Device(n_read_product_type))
        ...
        if (read_result)
        {
            ...
            for (int i = 0;i < BAC_ALALOG_CUSTMER_RANGE_TABLE_COUNT;i++)
            {
                ...
                memcpy(&m_analog_custmer_range.at(i), &read_data_buffer[i * 53], sizeof(Str_table_point));//nput 只个字到1个 b的寄存器里面;
            }
    ...
    case READTIMESCHEDULE_T3000:
    {
        ...
        read_result = true;
        //weekly_list_line
        //for (int i = 0; i < schedule_time_reg; i++) //schedule_time_reg 1读1包
        //{
    ...
    case   READ_SETTING_COMMAND:
    {
        ...
        if (read_result)
        {
            ...
            memcpy(&Device_Basic_Setting.reg, read_data_buffer, 400); //Setting 00个
	...
	{
		//cus table  106 06算*5    106x5  需要  需要读取
		for (int i = 0; i < 6; i++)
		{
			int itemp = 0;
		...
		if (read_result)
		{
			...
			for (int i = 0; i < BAC_ALALOG_CUSTMER_RANGE_TABLE_COUNT; i++)
			{
				...
				memcpy(&m_analog_custmer_range.at(i), &read_data_buffer[i * 53], sizeof(Str_table_point));//tr_table_point 只6个字到1个 b的寄存器里面;
			}
